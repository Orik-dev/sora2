...
async def menu_text(cb: CallbackQuery, state: FSMContext):
    async with SessionLocal() as session:
        ...
        # ✅ Для Text-to-Video ОСТАВЛЯЕМ выбор формата
        await state.update_data(ar="9:16", model="sora2-t2v")
        await edit_or_send(
            cb, 
            "✏️ Введите описание того, что должно происходить в видео:", 
...
async def toggle_ar(cb: CallbackQuery, state: FSMContext):
    ...
    await edit_or_send(
        cb, 
        "✏️ Введите описание того, что должно происходить в видео:", 
        reply_markup=kb_vertical_toggle(is_vertical=(new_ar=="9:16"))
    )
    await safe_cb_answer(cb)
...
async def set_prompt(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await redis.enqueue_job(
            ...
            prompt=txt,
            model="sora2-t2v",
            aspect_ratio=ar,  # ✅ Для текста передаём выбранный формат
            images_list=None,
        )
    except Exception:
        with contextlib.suppress(Exception): await processing.delete()
